...
const char* baseString = "0123456789";
	...
	for (buffer_id = 0; buffer_id < TX_BUFFER_COUNT; buffer_id += BUFFER_INCREMENT) {
		...
		if (verify)
			for (i = 0; i < 1; i++) // test_size / sizeof(unsigned int); i++)
				channel_ptr->buf_ptr[buffer_id].buffer[i] = *buffer;
...
// Function to initialize a char* pointer with baseString repeated until it reaches the specified size
char* initializeBuffer(int bufferSize, const char* baseString) {
    char* buffer = new char[bufferSize + 1]; // +1 for null terminator
    int baseLength = std::strlen(baseString);
    int repetitions = bufferSize / baseLength;
    int remainingChars = bufferSize;
    char* currentPos = buffer;

    for (int i = 0; i < repetitions; ++i) {
        std::memcpy(currentPos, baseString, baseLength);
        currentPos += baseLength;
        remainingChars -= baseLength;
    }

    // Fill the remaining part of the buffer if any
    if (remainingChars > 0) {
        std::memcpy(currentPos, baseString, remainingChars);
    }
...
{   
    buffer = initializeBuffer(bufferSize, baseString);
	int i;
